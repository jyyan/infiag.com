...
const introByLocale = computed(() => ({
  'zh-TW': '我們是一個專注於 AI/XR 開放生態系的平台公司。透過開放專利與技術規範，打破當前市場的閉鎖生態，讓硬體廠、軟體商與消費者三方共贏。',
  'zh-CN': '我们是一个专注于 AI/XR 开放生态系的平台公司。透过开放专利与技术规范，打破当前市场的闭锁生态，让硬件厂、软件商与消费者三方共赢。',
  'en':    'We are a platform company focused on the open AI/XR ecosystem. Through open patents and technical standards, we break the current locked ecosystem and create a win-win for hardware makers, software developers, and consumers.',
}[locale.value] ?? ''))

const soar = computed(() => [
  { letter: 'S', title: locale.value === 'en' ? 'Strengths' : '優勢',
    desc: locale.value === 'en' ? 'Open XR/AI tech foundation, precise niche entry'
                                 : '開放技術底座、精準利基切入' },
  ...
  { letter: 'A', title: locale.value === 'en' ? 'Aspirations' : '抱負',
    desc: locale.value === 'en' ? 'Become the Qualcomm/Android of AI/XR'
                                 : '成為 AI/XR 界的「高通」與「安卓」' },
    ...
    <section class="relative pt-28 pb-8 lg:pt-36 lg:pb-12 grid-tech overflow-hidden">
      <div class="container-tight flex justify-center">
        <div
          ...
          class="relative"
        >
          <div class="absolute inset-4 rounded-full blur-2xl bg-accent-glow/25 animate-pulse-glow" />
          <img
            src="/logo.png"
